...
do_install() {
	     ...
	     cp -vrfp ${FILE_DIRNAME}/rpioverlay-0.1/files/etc/wpa_supplicant ${D}${sysconfdir}/
	     mkdir -p ${D}${sysconfdir}/systemd/system
	     cp -vrf  ${FILE_DIRNAME}/rpioverlay-0.1/files/lib/systemd/system/network@wlan0.service ${D}${sysconfdir}/systemd/system/
}
...
SYSROOT_PREPROCESS_FUNCS += "rpioverlay_sysroot_preprocess"

rpioverlay_sysroot_preprocess () {
    ...
    cp -vrf  ${FILE_DIRNAME}/rpioverlay-0.1/files/bcm2835-bootfiles/cmdline.txt ${DEPLOY_DIR_IMAGE}/bcm2835-bootfiles/
}
